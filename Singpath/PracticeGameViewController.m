...
-(void)nextQuestions{
    ...
    if([questionChecker isCorrect:[selectedChoicesViewController selectedChoices]] && [autoAdvance isOn]){
        if([questionChecker counter]+1>=[[questionChecker questions] count]){
            ...
            LevelEndViewController *levc=[[LevelEndViewController alloc] initWithTitle:@"lvlEnd" navController:[self navigationController] stars:0 levelId:self.problemSetId pathId:self.pathID check:FALSE];
            levc.delegate = self;
