...
@synthesize pageViews = _pageViews,tableData=_tableData,currentView=_currentView,stories=_stories,storyIDs=_storyIDs,imageView=_imageView,heading,backgrond,arrowImg;
...
- (void)loadPage:(NSInteger)page {
    ...
    if ((NSNull*)pageView == [NSNull null]) {
        ...
        [redButton addTarget:self action:@selector(selectStory:) forControlEvents:UIControlEventTouchUpInside];
        
        [self.scrollView addSubview:redButton];
        [self.pageViews replaceObjectAtIndex:page withObject:redButton];
    }
}
-(void)selectStory: (id)sender
{
    ...
    pathViewController.storyName=[self.tableData objectAtIndex:self.pageControl.currentPage];
    pathViewController.storyid=[self.storyIDs objectAtIndex:self.pageControl.currentPage];
...
- (void)viewDidLoad {
   ...
   // [self.heading setAdjustsFontSizeToFitWidth:TRUE];
    ...
    self.stories=[Story getAllStories];
    
  //  self.paths=[Paths getAllPaths];
    NSMutableArray *storyNames=[[NSMutableArray alloc] init];
    NSMutableArray *storyIds=[[NSMutableArray alloc] init];
    
    for(int i=0;i<self.stories.count;i++){
        Story *currentStory=[self.stories objectAtIndex:i];
     ...
     //   }else{
            [storyNames addObject:[currentStory name]];
            [storyIds addObject:[currentStory id]];
            
     //   }
    }
    
    self.storyIDs=[NSArray arrayWithArray:storyIds];
    self.tableData=[NSArray arrayWithArray:storyNames];
